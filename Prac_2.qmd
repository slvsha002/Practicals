...
```{r}
# Question 3: Comparing with R's built in 'lowess()' function using ggplot2
library(ggplot2)

# Generate the LOWESS smoothed data using the built-in function
...
ggplot(df, aes(x = x, y = y)) + 
  geom_point(color = "blue", size = 2) +  # Scatter plot for original data
  geom_line(aes(x = x_sorted, y = y_smooth_sorted), color = "green", size = 1.5) +  # Custom LOWESS line
  geom_line(aes(x = x_sorted, y = y_builtin), color = "red", size = 1.5, linetype = "dashed") +  # R lowess() line
